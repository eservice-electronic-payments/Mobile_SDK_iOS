...
protocol ThreeDS2VerificationServiceProtocol {
    ...
    func startChallange(
        _ threeDS2Challange: ThreeDS2Challange,
        timeout: Int,
...
/**
 * Service that wraps the NSoftware SDK and handles 3DSV2 challanges for credit cards.
...
final class ThreeDS2VerificationService: ThreeDS2VerificationServiceProtocol {
    ...
    /// ViewController that will be used for displaying the challange
    private let viewController: ViewControllerProtocol

    /// Transaction generated by the transaction builder. Instance of this type will create a challange
    private var currentTransaction: TransactionProtocol?

    /// Action that will be executed asynchronously after the result of the challange will be known
     ...
     *   decide if the challange is needed, or is it a frictionless payment. This data allows
     *   webview to generate a challange
     */
    func generateRequest() throws -> PaymentRequest {
        do {
     ...
     * Generate new transaction and extract all the data from the transaction into a `PaymentRequest`
     * - Parameters:
     *      - threeDS2Challange: All the data needed to generate a challange for the user
     *      - timeout: Number of seconds for the challange to load after which the payment will fail
     *      - completion: Action that needs to be executed after the result of challange is known.
    ...
    func startChallange(
        ...
        let challengeParameters = ChallengeParameters(
            threeDSServerTransactionID: threeDS2Challange.threeDSServerTransactionID,
            acsTransactionID: threeDS2Challange.acsTransactionID,
            acsRefNumber: threeDS2Challange.acsRefNumber,
            acsSignedContent: threeDS2Challange.acsSignedContent,
...
/// Implementation of `ChallengeStatusReceiver` protocol that handles the result of a challange once it is known
